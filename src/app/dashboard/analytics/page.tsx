      ...
      <div className="flex flex-col gap-4 py-4 sm:gap-6 sm:py-6">
        ...
        <Card className="mx-4 lg:mx-6">
          ...
          <CardContent>
            ...
            <div>
              <p className="text-xs text-muted-foreground sm:text-sm">Total SKUs</p>
              <p className="text-xl font-bold sm:text-2xl">
                {inventoryStats.data?.data?.totalSKUs || 0}
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground sm:text-sm">Total Quantity</p>
              <p className="text-xl font-bold sm:text-2xl">
            ...
            <div>
              <p className="text-xs text-muted-foreground sm:text-sm">Average Quantity</p>
              <p className="text-xl font-bold sm:text-2xl">
          ...
          {topSKUs.data?.data && topSKUs.data.data.length > 0 && (
            <div className="mt-6">
              <h3 className="text-base font-semibold mb-4 sm:text-lg">Top 10 SKUs by Quantity</h3>
              <div className="rounded-lg border overflow-x-auto">
                <Table>
                  <TableHeader>
                    <TableRow>
                      <TableHead className="h-10 min-w-[100px]">SKU</TableHead>
                      <TableHead className="h-10 min-w-[150px]">Name</TableHead>
                      <TableHead className="h-10 min-w-[100px]">Quantity</TableHead>
                      <TableHead className="h-10 min-w-[100px] hidden sm:table-cell">Location</TableHead>
                  ...
                  <TableBody>
                    {topSKUs.data.data.map((sku) => (
                      <TableRow key={sku.sku} className="hover:bg-muted/50">
                        <TableCell className="font-mono text-xs sm:text-sm">{sku.sku}</TableCell>
                        <TableCell className="text-xs sm:text-sm">{sku.name}</TableCell>
                        <TableCell className="tabular-nums text-xs sm:text-sm">{sku.quantity.toLocaleString()}</TableCell>
                        <TableCell className="text-muted-foreground text-xs sm:text-sm hidden sm:table-cell">{sku.location || "-"}</TableCell>
                        <TableCell>
                          <Badge
                            variant={sku.isLowStock ? "destructive" : "outline"}
        ...
        <Card className="mx-4 lg:mx-6">
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4 sm:grid-cols-4 mb-6">
            <div>
              <p className="text-xs text-muted-foreground sm:text-sm">Total Labels</p>
              <p className="text-xl font-bold sm:text-2xl">
                {labelStats.data?.data?.total || 0}
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground sm:text-sm">Success Rate</p>
              <p className="text-xl font-bold sm:text-2xl">
                {labelStats.data?.data?.successRate.toFixed(1) || 0}%
              </p>
            </div>
            <div>
              <p className="text-xs text-muted-foreground sm:text-sm">Total Cost</p>
              <p className="text-xl font-bold sm:text-2xl">
            ...
            <div>
              <p className="text-xs text-muted-foreground sm:text-sm">Avg Cost/Label</p>
              <p className="text-xl font-bold sm:text-2xl">
